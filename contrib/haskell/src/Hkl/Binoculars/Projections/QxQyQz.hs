...
data instance Config 'QxQyQzProjection = BinocularsConfigQxQyQz
    ...
    , _binocularsConfigQxQyQzProjectionResolution   :: Resolutions DIM3
    , _binocularsConfigQxQyQzProjectionLimits       :: Maybe (RLimits DIM3)
...
instance HasIniConfig 'QxQyQzProjection where
  ...
  defaultConfig = BinocularsConfigQxQyQz
    ...
    , _binocularsConfigQxQyQzProjectionResolution = Resolutions3 0.01 0.01 0.01
  ...
  overwriteWithCmd mr = over binocularsConfigQxQyQzInputRange (mr <|>)
...
{-# INLINE spaceQxQyQz #-}
spaceQxQyQz :: Detector a DIM2 -> Array F DIM3 Double -> Resolutions DIM3 -> Maybe Mask -> SurfaceOrientation -> Maybe (RLimits DIM3) -> Space DIM3 -> DataFrameQCustom -> IO (DataFrameSpace DIM3)
...
class (FramesQCustomP a, Show a) => ProcessQxQyQzP a where
  ...
  processQxQyQzP mkPaths = do
    ...
    let res = _binocularsConfigQxQyQzProjectionResolution conf
