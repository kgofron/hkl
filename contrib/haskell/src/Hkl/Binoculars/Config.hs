...
getConfig mf = do
  cfg <- readFile =<< case mf of
                       ...
                       (Just f) -> pure f
  --  TODO, some files generated by users have this [section]## They
  --  are not parsable by haskell, but there is not problem with
  --  Python. It would be great to remove the trailling '##'
  let cfg' = unlines $ [fixHeader l | l <- lines cfg]
