                                ...
                                double seuil, double ADU_per_photon, int cosmic, int contour)
{
	...
	if (dark->type != XRAYS_IMAGE_USHORT
		...
		droplet = malloc(sizeof(XRaysDroplet));
		if(!droplet)
			die("can not allocate XRaysDroplet");
 ...
 */
static void droplet_intensity_and_coordinates(XRaysDroplet *droplet, unsigned short int const *imgs, int **gtt, int **contour, float *intensity, unsigned int *x, unsigned int *y)
{
	unsigned int index;
