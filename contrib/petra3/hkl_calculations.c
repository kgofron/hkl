...
HklEngine *new_hkl_engine(char *diffr_type, HklGeometry *geometry, HklDetector *detector, HklSample *sample)
{
...
void hklmatrix_to_gslmatrix(HklMatrix *m_hkl, gsl_matrix *m_gsl)
{
    int i, j;
    double x;
...
void calculate_M_matrix(HklGeometry *geometry, HklDetector *detector, gsl_matrix *M)
{
...
void compute_transf_matrix(double *refl_hkl, double trans_matrix[3][3])
{
    ...
    geometry = hkl_geometry_list_item_geometry_get(hkl_geometry_list_items_first_get(geometries));

    /* The values of the angles (4S + 2D) can be obtained with */
    hkl_geometry_axis_values_get(geometry, angles, n_angles, HKL_UNIT_USER);
    ...
    calculate_M_matrix(geometry, detector, M);

    /* Matrix transformation from recipr to BG reference frame */
    R_quat = hkl_geometry_sample_rotation_get(geometry, sample);
