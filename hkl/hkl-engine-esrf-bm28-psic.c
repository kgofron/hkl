 ...
 *
 * Copyright (C) 2003-2017, 2025 Synchrotron SOLEIL
...
#define HKL_GEOMETRY_ESRF_BM28_PSIC_DESCRIPTION				\
...
static const char* hkl_geometry_ESRF_BM28_PSIC_axes[] = {MU, ETA, CHI, PHI, NU, DELTA};

static HklGeometry *hkl_geometry_new_ESRF_BM28_PSIC(const HklFactory *factory)
{
...
{
	static const char* axes_r[] = {MU, ETA, CHI, PHI, NU, DELTA};
...
REGISTER_READONLY_INCIDENCE(hkl_engine_incidence_esrf_bm28_psic_new,
			    P99_PROTECT({MU, ETA, CHI, PHI}),
			    surface_parameters_y);

REGISTER_READONLY_EMERGENCE(hkl_engine_emergence_esrf_bm28_psic_new,
			    P99_PROTECT({MU, ETA, CHI, PHI, NU, DELTA}),
			    surface_parameters_y);
...
static HklEngine *hkl_engine_hkl_esrf_bm28_psic_new(HklEngineList *engines)
{
	HklEngine *self;
...
static HklEngineList *hkl_engine_list_new_ESRF_BM28_PSIC(const HklFactory *factory)
{
	HklEngineList *self = hkl_engine_list_new();

	hkl_engine_hkl_esrf_bm28_psic_new(self);
	hkl_engine_incidence_esrf_bm28_psic_new(self);
	hkl_engine_emergence_esrf_bm28_psic_new(self);

	return self;
}

/* Register the diffractometer into the factory */
REGISTER_DIFFRACTOMETER(ESRF_BM28_PSIC, "ESRF BM28 PSIC", HKL_GEOMETRY_ESRF_BM28_PSIC_DESCRIPTION);
