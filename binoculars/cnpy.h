...
namespace cnpy {
    ...
    template<typename T> void npz_save(std::string zipname, std::string fname, const T* data, const std::vector<size_t>& shape, std::string mode = "w")
    {
        ...
        if(fp) {
            ...
            fseek(fp, static_cast<long int>(global_header_offset),SEEK_SET);
        ...
        uint32_t crc = static_cast<uint32_t>(crc32(static_cast<uLong>(0L),(uint8_t*)&npy_header[0],static_cast<uInt>(npy_header.size())));
    ...
    template<typename T> std::vector<char> create_npy_header(const std::vector<size_t>& shape) {
        ...
        dict.insert(dict.end(), static_cast<std::vector<char>::size_type>(remainder),' ');
        dict.back() = '\n';
